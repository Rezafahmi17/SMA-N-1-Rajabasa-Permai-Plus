...
exports.create = async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ success: false, message: 'Foto wajib diupload', data: null });
    const { judul } = req.body;
    const item = await Galeri.create({ judul: judul || '', foto: `/uploads/${req.file.filename}` });
...
exports.remove = async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ success: false, message: 'Foto tidak ditemukan', data: null });
    await item.destroy();
