        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
          ...
          {loading && (
            <div className="space-y-4 animate-pulse">
              ...
              {[1,2,3,4].map(i => <div key={i} className="h-4 bg-navy-100 rounded-md" />)}
            </div>
          )}
          ...
          {error && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="font-serif text-xl font-semibold text-navy-700 mb-2">Berita Tidak Ditemukan</h3>
              <p className="text-sm text-navy-400">{error}</p>
              <Link to="/" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-navy-700 px-6 py-2.5 text-sm font-semibold text-white hover:bg-navy-600 transition-colors">
                Kembali ke Beranda
              </Link>
            </div>
          )}
          ...
          {berita && (
            <article className="animate-fade-up">
              ...
              <div className="prose-content">
                <p className="text-base text-navy-500 leading-[1.9] whitespace-pre-line">{berita.isi}</p>
              </div>

              {/* Footer share */}
              <div className="mt-10 flex items-center gap-4 border-t border-navy-100 pt-6">
                <span className="text-sm font-semibold text-navy-600">Bagikan:</span>
                <div className="flex gap-2">
                  {[
                    ...
                    <button key={i} title={btn.name} className="flex h-9 w-9 items-center justify-center rounded-xl bg-navy-50 text-navy-400 transition-all hover:bg-navy-100 hover:text-navy-600 hover:-translate-y-0.5">
                      {btn.icon}
                    </button>
                  ))}
                </div>
