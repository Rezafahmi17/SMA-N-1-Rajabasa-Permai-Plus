    ...
    <>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {items.map((item, idx) => (
          ...
          >
            <div className={idx % 7 === 0 ? 'h-full min-h-[200px]' : 'aspect-square'}>
              <img
            ...
            {item.judul && (
              <div className="absolute bottom-0 left-0 right-0 translate-y-full bg-gradient-to-t from-navy-900/90 to-transparent px-3 py-2 transition-transform duration-300 group-hover:translate-y-0">
                <p className="text-xs font-medium text-white line-clamp-1">{item.judul}</p>
              </div>
            )}
          </button>
        ))}
          ...
          >
            ...
            {selected.judul && (
              <div className="mt-4 text-center">
                <p className="text-base font-medium text-white">{selected.judul}</p>
              </div>
            )}
          </div>
        </div>
